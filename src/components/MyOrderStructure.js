    ...
    <div className="flex flex-col items-center">
      {ongoingOrder && (
        ...
        >
          ...
          <h3 type="button" className="pt-[7px]">{`Ordered Date : ${date.toUTCString()}`}</h3>
          <Link to={`/report/${ongoingOrder._id}`}>
            <button type="button" className="mt-[7px] px-4 py-2 text-white bg-[#0098FF] rounded-[100px] hover:bg-sky-700">View Report</button>
          </Link>
        </div>
      )}
    </div>
